...
private String TAG="FilterStack";
    ...
    private void invalidate() {
        ...
        if (source != null) {
            ...
            photoView.setPhoto(source, topFilterOutputted);
        }
    }
    ...
    void pushFilterRedeye(Filter filter) {
    	if(appliedStack.size()==0)
    	{
         ...
         public void run() {
        	   for (int i = 0; i < buffers.length; i++) {
        	    ...
        	    if (source != null) {
        	        buffers[0] = Photo.create(source.width(), source.height());
        	        Photo input =  source;
        	        if ((input != null) && (buffers[0] != null)) {
        	            appliedStack.get(0).process(input, buffers[0]);
        	            photoView.setPhoto( buffers[0], topFilterOutputted);
        	        }
        	    }
         }
     });
    ...
    public void saveRedEyeBitmap(final OnDoneBitmapCallback callback) {
        photoView.queue(new Runnable() {
