...
public class MosaicFrameProcessor {
    ...
    public static final int MAX_NUMBER_OF_FRAMES = 100;
    ...
    public void initialize(boolean is3D) {
        setupMosaicer(mPreviewWidth, mPreviewHeight, mPreviewBufferSize,is3D);
        setStripType(Mosaic.STRIPTYPE_WIDE);
        reset();
    }
    public void clear(boolean is3D) {
        if (mIsMosaicMemoryAllocated) {
            mIsMosaicMemoryAllocated = false;
            if(CSStaticData.DEBUG)
            Log.e(TAG, "----------------------[清除了mosaic内存——C]clearMosaicFrameProcessorIfNeeded------------------");
            mMosaicer.freeMosaicMemory3D(is3D);
        }
    }
    ...
    private void setupMosaicer(int previewWidth, int previewHeight, int bufSize,boolean is3D) {
        Log.v(TAG, "setupMosaicer w, h=" + previewWidth + ',' + previewHeight + ',' + bufSize);
       //Log.e("alloc 1", "ok");
        mMosaicer.allocateMosaicMemory3D(previewWidth, previewHeight,is3D);
       // Log.e("alloc 2", "ok");
        mIsMosaicMemoryAllocated = true;
    ...
    public int createMosaic(boolean highRes,boolean isleft) {
        return mMosaicer.createMosaic3D(highRes,isleft);
    }
    public int createMosaic2D(boolean highRes) {
        return mMosaicer.createMosaic(highRes);
    }
    public int[] getFinalMosaicInt() {
        return mMosaicer.getFinalMosaic();
    }
    ...
    public void processFrame(byte[] prebyte ) {
        if (!mIsMosaicMemoryAllocated) {
            // clear() is called and buffers are cleared, stop computation.
            // This can happen when the onPause() is called in the activity, but still some frames
            // are not processed yet and thus the callback may be invoked.
        ...
        if (mCurrProcessFrameIdx != mLastProcessFrameIdx) {
            ...
            if (mTotalFrameCount < MAX_NUMBER_OF_FRAMES) {
                // If we are still collecting new frames for the current mosaic,
                // process the new frame.
                calculateTranslationRate(timestamp,prebyte);
    ...
    public void processFrame(byte[] prebyte,byte[] prebyte_ ) {
//    	Log.e(TAG, "processFrame  ++++++++++++   --------------------");
//    	Log.e(TAG, "mTotalFrameCount="+mTotalFrameCount);
//    	Log.e(TAG, "mIsMosaicMemoryAllocated="+mIsMosaicMemoryAllocated);
//    	Log.e(TAG, "mCurrProcessFrameIdx="+mCurrProcessFrameIdx);
//    	Log.e(TAG, "mLastProcessFrameIdx="+mLastProcessFrameIdx);
        if (!mIsMosaicMemoryAllocated) {
            // clear() is called and buffers are cleared, stop computation.
            // This can happen when the onPause() is called in the activity, but still some frames
            // are not processed yet and thus the callback may be invoked.
        ...
        if (mCurrProcessFrameIdx != mLastProcessFrameIdx) {
            ...
            if (mTotalFrameCount < MAX_NUMBER_OF_FRAMES) {
                // If we are still collecting new frames for the current mosaic,
                // process the new frame.
                calculateTranslationRate(timestamp,prebyte,prebyte_);
    ...
    public void calculateTranslationRate(long now,byte[] prebyte,byte[] prebyte_) {
    //	Log.e("++++++++++++++++++++++", "ok");
        float[] frameData = mMosaicer.setSourceImage3D(prebyte,prebyte_);
      //  Log.e("________________________", "ok");
      //  Log.e("setSourceImage   ̎��mosaic��id ", ""+mMosaicer);
        ...
        if(CSStaticData.DEBUG)
        Log.e("frame changdu +++++++++++", "="+frameData[9]);
        curframe=(int) frameData[9];
    ...
    public void calculateTranslationRate(long now,byte[] prebyte) {
        float[] frameData = mMosaicer.setSourceImage(prebyte);
        int ret_code = (int) frameData[MOSAIC_RET_CODE_INDEX];
        mTotalFrameCount  = (int) frameData[FRAME_COUNT_INDEX];
        float translationCurrX = frameData[X_COORD_INDEX];
        float translationCurrY = frameData[Y_COORD_INDEX];
